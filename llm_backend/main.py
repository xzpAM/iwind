from fastapi import FastAPI, HTTPException, UploadFile, File, Query, Form, Request, BackgroundTasks
...
async def process_prompt_with_session(prompt: str, session_id: str, images: List[Dict] = None):
    global_variables.set_current_session(session_id)
    logger.info(global_variables.get_current_session())
    logger.info(f"Processing prompt for session {session_id}")
    try:
        agent = global_variables.get_agent(session_id, images=images)
        await agent.run(prompt)
    except Exception as e:
        logger.error(f"Prompt error in session {session_id}: {str(e)}")
...
def build_enhanced_prompt(base_prompt: str, images: list) -> str:
    ...
    return (
        f"{base_prompt}\n\n"
        f"### 附加图片信息（共 {len(images)} 张）：\n"
        f"{chr(10).join(image_descriptions)}\n\n"
        "请对上述图片内容进行风机损伤检测。\n"
        "注意：回答中**不要输出任何图片路径或本地文件路径**，"
        "避免出现“假设”或“路径应该是”之类的表述"
        "只能描述检测结果"
    )

# -------------------- 文件上传接口（多文件+session_id） --------------------
...
async def upload_files(
    ...
    saved_files = []
    session_dir = os.path.join(str(UPLOAD_DIR), session_id)
    try:
...
async def answer_question(request: Request):
    ...
    try:
        logger.info(f"answer_session_id: {session_id}")
        q = global_variables.get_input_queue(session_id)
        await global_variables.clear_asyncio_queue(q)
        await q.put(prompt)
        return {"status": "received"}
    except Exception as e:
        logger.error(f"Error sending prompt to session {session_id}: {str(e)}")
...
async def send_message(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.error(f"Error starting prompt processing: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def cancel_stream(request: Request):
    ...
    try:
        ...
        if agent:
            ...
            logger.info(f"Agent资源已释放 for session {session_id}")
        return {"status": "cancelled"}
    except Exception as e:
        logger.error(f"Cancel failed for {session_id}: {str(e)}")
...
async def proxy_file(url: str = Query(...)):
    ...
    try:
        ...
        return StreamingResponse(content=f"Proxy error: {str(e)}", status_code=502)
