...
# rainbow_yu openfast.tool.openfast_5MW_Land_ModeShapes 🐋✨

from deepwind_app.tool.base import BaseTool
import requests

_openfast_5MW_Land_ModeShapes_DESCRIPTION = """这个工具是用来：这是5MW风力发电机在陆地环境下的模式形状工具，用于分析风力机结构的振动模式，帮助优化风机设计以提高稳定性。并且模拟结果后对结果进行总结。
"""

class openfast_5MW_Land_ModeShapes(BaseTool):
    name: str = "openfast_5MW_Land_ModeShapes"
    description: str = _openfast_5MW_Land_ModeShapes_DESCRIPTION
    parameters: dict = {
    "type": "object",
    "properties": {
        "fst": {
            "type": "object",
            "description": "Parameters extracted from 5MW_Land_ModeShapes.fst",
            "properties": {
                "Echo": {
                    "type": "boolean",
                ...
                "WrVTK": {
                    "type": "number",
                    "description": "VTK visualization data output: (switch) {0=none; 1=initialization data only; 2=animation; 3=mode shapes}"
                },
                "VTK_type": {
                    "type": "number",
    ...
    async def execute(self, **kwargs):
        try:
            ...
            url = "http://localhost:8002/openfast/r_test/openfast_5MW_Land_ModeShapes"
