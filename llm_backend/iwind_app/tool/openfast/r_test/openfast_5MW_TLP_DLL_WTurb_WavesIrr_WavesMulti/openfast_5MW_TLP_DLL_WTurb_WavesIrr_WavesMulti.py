...
# rainbow_yu openfast.tool.openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti 🐋✨

from deepwind_app.tool.base import BaseTool
import requests

_openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti_DESCRIPTION = """OpenFAST 5MW TLP (Tension Leg Platform) 风机仿真工具，支持风-涡旋-波浪-多重波浪场耦合计算。

该工具基于NREL OpenFAST框架，专门用于模拟半潜式张力腿平台（TLP）型海上风电机组的动态响应。并且模拟结果后对结果进行总结。
"""

class openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti(BaseTool):
    name: str = "openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti"
    description: str = _openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti_DESCRIPTION
    parameters: dict = {
    "type": "object",
    "properties": {
        "fst": {
            "type": "object",
            "description": "Parameters extracted from 5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti.fst",
            "properties": {
                "Echo": {
                    "type": "boolean",
        ...
        "elastodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_ElastoDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo input data to \"<RootName>.ech\" (flag)"
        ...
        "tower": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_ElastoDyn_Tower.dat",
            "properties": {
                "NTwInpSt": {
                    "type": "number",
        ...
        "feamooring": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_FEAMooring.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo input data to <RootName>.ech (flag)"
                ...
                "27.00_LRadAnch_(m)": {
                    "type": "array",
                    "description": "Table data columns: 27.00, LRadAnch, (m)",
                    "items": {
                        "type": "object",
                        "properties": {
                            "27.00": {
        ...
        "hydrodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_HydroDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
        ...
        "map": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_MAP.dat",
            "properties": {
        ...
        "moordyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_MoorDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "echo the input file data (flag)"
        ...
        "servodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_MIT_NREL_TLP_ServoDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo input data to <RootName>.ech (flag)"
    ...
    async def execute(self, **kwargs):
        try:
            # 包装成服务端预期格式
            payload = {"updates": {}}
            for module_name in ['fst', 'elastodyn', 'tower', 'feamooring', 'hydrodyn', 'map', 'moordyn', 'servodyn', 'aerodyn', 'aerodyn15']:
                if module_name in kwargs:
                    payload["updates"][module_name] = kwargs[module_name]

            url = "http://localhost:8002/openfast/r_test/openfast_5MW_TLP_DLL_WTurb_WavesIrr_WavesMulti"
