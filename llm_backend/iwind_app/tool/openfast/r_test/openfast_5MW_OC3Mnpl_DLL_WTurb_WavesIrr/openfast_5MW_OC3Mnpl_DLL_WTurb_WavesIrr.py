...
# rainbow_yu openfast.tool.openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr 🐋✨

from deepwind_app.tool.base import BaseTool
import requests

_openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr_DESCRIPTION = """这个工具是用来：这是OC3平台上的5MW风力发电机模型，适用于不规则波浪的动态模拟，用于海上风电场中浮式平台（OC3 Mnpl）的风机性能分析。并且模拟结果后对结果进行总结。
"""

class openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr(BaseTool):
    name: str = "openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr"
    description: str = _openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr_DESCRIPTION
    parameters: dict = {
    "type": "object",
    "properties": {
        "fst": {
            "type": "object",
            "description": "Parameters extracted from 5MW_OC3Mnpl_DLL_WTurb_WavesIrr.fst",
            "properties": {
                "Echo": {
                    "type": "boolean",
        ...
        "aerodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_AeroDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                ...
                "\"RtAeroFxh,_RtAeroFyh,_RtAeroFzh\"": {
                    "type": "array",
                    "description": "Table data columns: \"RtAeroFxh,, RtAeroFyh,, RtAeroFzh\"",
                    "items": {
                        "type": "object",
                        "properties": {}
                    }
        ...
        "aerodyn15": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_AeroDyn15.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo the input to \"<rootname>.AD.ech\"?  (flag)"
        ...
        "elastodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_ElastoDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo input data to \"<RootName>.ech\" (flag)"
                ...
                "PtfmSway\"": {
                    "type": "array",
                    "description": "Platform horizontal surge displacement"
                },
                "Spn2MLyb1\"": {
                    "type": "array",
        ...
        "tower": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_ElastoDyn_Tower.dat",
            "properties": {
                "NTwInpSt": {
                    "type": "number",
        ...
        "hydrodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_HydroDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
        ...
        "withice": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_HydroDyn_withIce.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo the input file data (flag)"
                ...
                "WaveMod": {
                    "type": "number",
                    "description": "Incident wave kinematics model {0: none=still water, 1: regular (periodic), 1P#: regular with user-specified phase, 2: JONSWAP/Pierson-Moskowitz spectrum (irregular), 3: White noise spectrum (irregular), 4: user-defined spectrum from routine UserWaveSpctrm (irregular), 5: Externally generated wave-elevation time series, 6: Externally generated full wave-kinematics time series [option 6 is invalid for HasWAMIT = TRUE]} (switch)"
                },
                "WaveStMod": {
                    "type": "number",
                    "description": "Model for stretching incident wave kinematics to instantaneous free surface {0: none=no stretching, 1: vertical stretching, 2: extrapolation stretching, 3: Wheeler stretching} (switch) [unused when WaveMod=0 or when HasWAMIT = TRUE]"
                },
                "WaveTMax": {
                    "type": "number",
                    "description": "Analysis time for incident wave calculations (sec) [unused when WaveMod=0; determines WaveDOmega=2Pi/WaveTMax in the IFFT]"
                ...
                "RdtnDT": {
                    "type": "number",
                    "description": "Time step for wave radiation kernel calculations (sec) [only used when PotMod=1; DT<=RdtnDT<=0.1 recommended; determines RdtnOmegaMax=Pi/RdtnDT in the cosine transform]"
                },
                "NBody": {
                    "type": "number",
                    "description": "Number of WAMIT bodies to be used (-) [>=1; only used when PotMod=1. If NBodyMod=1, the WAMIT data contains a vector of size 6*NBody x 1 and matrices of size 6*NBody x 6*NBody; if NBodyMod>1, there are NBody sets of WAMIT data each with a vector of size 6 x 1 and matrices of size 6 x 6]"
                ...
                "WAMITULEN": {
                    "type": "number",
                    "description": "Characteristic body length scale used to redimensionalize WAMIT output (meters) [only used when PotMod=1]"
                },
                "PtfmRefxt": {
                    "type": "number",
                    "description": "The xt offset of the body reference point(s) from (0,0,0) (meters) [1 to NBody] [only used when PotMod=1]"
        ...
        "servodyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_ServoDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                    "description": "Echo input data to <RootName>.ech (flag)"
        ...
        "subdyn": {
            "type": "object",
            "description": "Parameters extracted from NRELOffshrBsline5MW_OC3Monopile_SubDyn.dat",
            "properties": {
                "Echo": {
                    "type": "boolean",
                ...
                "M2N1MKye\"": {
                    "type": "array",
                    "description": "The local side-to-side and fore-aft bending moments at node 1 of member 3 (located at 0 m, i.e. MSL)."
                },
                "M1N1MKye\"": {
                    "type": "array",
                    "description": "The local side-to-side and fore-aft bending moments at node 1 of member 2 (located at -10 m, i.e. half way between MSL and mudline)."
                },
                "-ReactFYss,": {
                    "type": "array",
                    "description": "ReactFZss\" - Base reactions: fore-aft shear, side-to-side shear and vertical forces at the mudline."
                },
                "-ReactMYss,": {
                    "type": "array",
                    "description": "ReactMZss\" - Base reactions: side-to-side, fore-aft and yaw moments at the mudline."
                }
            }
        }
    },
    ...
    async def execute(self, **kwargs):
        try:
            # 包装成服务端预期格式
            payload = {"updates": {}}
            for module_name in ['fst', 'aerodyn', 'aerodyn15', 'elastodyn', 'tower', 'hydrodyn', 'withice', 'servodyn', 'subdyn']:
                if module_name in kwargs:
                    payload["updates"][module_name] = kwargs[module_name]

            url = "http://localhost:8002/openfast/r_test/openfast_5MW_OC3Mnpl_DLL_WTurb_WavesIrr"
