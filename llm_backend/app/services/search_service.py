...
class SearchService:
    ...
    async def _call_with_tool(self, messages: List[Dict]) -> Dict:
        ...
        try:
            ...
            if not isinstance(messages, list) or not all(
                ...
                raise ValueError("messages必须是由role和content组成的字典列表")

            logger.info("调用模型，消息: {}", messages)  # 使用安全日志格式化
            ...
            logger.info("模型返回: {}", response.choices[0].model_dump())
            return response.choices[0]  # 返回结构化数据

        except Exception as e:
            # 安全日志记录（避免f-string的格式化问题）
            logger.opt(exception=True).error("模型调用失败: {}", str(e))
            return {
                "error": str(e),
    ...
    async def generate_stream(
        ...
        try:
            ...
            logger.error("Error in _call_with_tool: %s", e, exc_info = True)
            raise